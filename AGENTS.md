...
This document is the **single source of truth** for all AI agents (Claude Code, Gemini CLI, etc.) working in the Tuturuuu monorepo. It defines repository-specific mental models, canonical workflows, hard guardrails, and collaboration protocols.

**Foundational Mandate**: Instructions here take absolute precedence. If a required rule is missing, propose an addition. **NEVER** invent ad-hoc behavior.
...
- **Least Privilege**: Touch ONLY the files required for the specific change.
...
### 5.3 Tailwind Dynamic Colors
- **Forbidden**: Static palette classes (e.g., `text-blue-500`).
- **Required**: `text-dynamic-blue`, `bg-dynamic-surface`, etc.

## 6. Command Palette

| Goal | Command | Scope |
| :--- | :--- | :--- |
| **Verify All** | `bun check` | Web/TS (Agent-Safe if requested) |
| **Verify Mobile** | `bun check:mobile` | Flutter (Agent-Allowed) |
| **Type Check** | `bun type-check` | Full Monorepo (Agent-Allowed) |
| **Fix Formatting** | `bun ff` | Biome (Agent-Safe if requested) |
| **Test Scoped** | `bun --filter <pkg> test` | Targeted (Agent-Allowed) |
| **New Migration** | `bun sb:new` | Database |
| **Update Types** | `bun sb:typegen` | Types (Post-Migration) |
| **Add Dependency** | `bun add <pkg>` | Scoped Package Dir |

## 7. Known Gotchas & Patterns
...
## 8. Continuous Improvement (Session Retrospective)
