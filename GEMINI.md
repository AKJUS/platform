...
- **Flutter Analyzer Hygiene:** Use `on Exception catch (e)` (or specific exception types) instead of bare `catch`, avoid catching `Error` subclasses like `TypeError`, capture `context` dependencies (for example `context.read<T>()`) before the first `await`, guard `BuildContext` usage after `await` with `if (!context.mounted) return;`, and do not `return` inside `finally` blocks.
- **Flutter Test Teardown:** In `addTearDown` async closures, return the delete future directly (`() async => tempDir.delete(...)`) instead of `await`-returning it to avoid `unnecessary_await_in_return` warnings.
...
- **render_ui First-Invalid Failsafe:** For first-try reliability, convert the first invalid `render_ui` payload into a deterministic valid failsafe UI immediately instead of emitting intermediate recovery placeholders.
- **render_ui Single-Result Rendering:** When one assistant turn contains multiple valid non-recovered `render_ui` outputs, render only the latest one in chat.
