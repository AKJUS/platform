...
- **Flutter Analyzer Hygiene:** Prefer `on Exception catch (e)` (or a specific type) over bare `catch`, avoid catching `Error` subclasses like `TypeError`, capture `context` dependencies (for example `context.read<T>()`) before the first `await`, guard `BuildContext` usage after `await` with `if (!context.mounted) return;`, and never `return` from a `finally` block.
- **Flutter Test Teardown:** In `addTearDown` async closures, return the future directly (`() async => tempDir.delete(...)`) instead of `await`-returning it to avoid `unnecessary_await_in_return` analyzer warnings.
