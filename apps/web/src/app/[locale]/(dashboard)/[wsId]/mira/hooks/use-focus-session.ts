...
import type { MiraAchievement, MiraFocusSession, MiraPet } from '../types/mira';
import { miraKeys } from './use-mira';

interface FocusResponse {
  active_session: MiraFocusSession | null;
  recent_sessions: MiraFocusSession[];
  stats: {
...
async function fetchFocusData(): Promise<FocusResponse> {
  const res = await fetch('/api/v1/mira/focus', { cache: 'no-store' });
  if (!res.ok) {
...
async function startFocusSession(data: {
  ...
  const res = await fetch('/api/v1/mira/focus/start', {
    method: 'POST',
...
async function completeFocusSession(data: {
  ...
  const res = await fetch('/api/v1/mira/focus/complete', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  });
...
async function fetchFocusHistory(
  ...
  const res = await fetch(
    `/api/v1/mira/focus/history?limit=${limit}&offset=${offset}`,
    { cache: 'no-store' }
  );
  if (!res.ok) {
...
async function deleteFocusSession(
  ...
  const res = await fetch(`/api/v1/mira/focus/${sessionId}`, {
    method: 'DELETE',
  });
  if (!res.ok) {
    const error = await res.json();
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({
        queryKey: [...miraKeys.all, 'focus', 'history'],
      });
    },
  });
}
...
export function useFocusHistory(limit = 20) {
  return useQuery({
    queryKey: miraKeys.focusHistory(limit),
