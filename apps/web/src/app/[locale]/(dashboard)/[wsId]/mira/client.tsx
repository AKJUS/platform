...
import { useFeedMira, useMira } from './hooks/use-mira';

import type { MiraAchievement, MiraAnimationState } from './types/mira';

interface MiraClientProps {
  wsId: string;
  isPersonal: boolean;
}

export default function MiraClient({ wsId, isPersonal }: MiraClientProps) {
  const { data: miraData, isLoading: isPetLoading } = useMira();
  useFocusSessions(); // Keep hook active for data caching
  const focusTimer = useFocusTimer();

  const feedMutation = useFeedMira();
  ...
  const [activeMode, setActiveMode] = useState<MiraMode>('home');
  const [showStartFocusDialog, setShowStartFocusDialog] = useState(false);
  const [showCompleteDialog, setShowCompleteDialog] = useState(false);
  const [completedSessionData, setCompletedSessionData] = useState<{
    xpEarned: number;
    achievements: MiraAchievement[];
  } | null>(null);
  const [chatMessage, setChatMessage] = useState<string | null>(null);
  const [animationState, setAnimationState] =
    useState<MiraAnimationState>('idle');
  const [isVoiceChatActive, setIsVoiceChatActive] = useState(false);

  const pet = miraData?.pet;

  // Calculate animation state based on focus and mood
  const currentAnimationState = useMemo((): MiraAnimationState => {
  ...
  const triggerAnimation = useCallback(
    (state: MiraAnimationState, duration = 2000) => {
  ...
  const handleFeed = useCallback(() => {
    feedMutation.mutate(undefined, {
      onSuccess: (data) => {
        ...
        toast.success(`Fed Mira! +${data.xp_earned} XP`);
      },
      onError: (error) => {
  ...
  // Handle mode changes
  const handleModeChange = useCallback((mode: MiraMode) => {
    setActiveMode(mode);
  }, []);
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center">
        <p className="text-muted-foreground">
          Failed to load Mira. Please refresh.
        </p>
      </div>
    );
  }
