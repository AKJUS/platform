...
Web QR sign-in approval is gated by this setting:
...
- The active mobile account is the account that approves the web login.
...
If a user wants to approve QR login for a different account, they must switch to
that account in the mobile app before scanning.

The same Settings > Session surface can approve web MFA challenges when the
active mobile Supabase session is already at `aal2`:

- The web MFA screen creates an `/api/v1/auth/mfa/mobile/challenges` request and
  shows a pair code.
- Mobile polls `/api/v1/auth/mfa/mobile/approvals` while the session settings
  screen is mounted, shows matching pair codes, and approves through
...
- Mobile sessions that are still `aal1` may see pending state, but the API will
  not approve a web MFA request until the mobile session has passed MFA.
