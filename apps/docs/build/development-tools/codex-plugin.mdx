...
`tuturuuu-platform`, displays as `Tuturuuu Platform`, and points its local plugin
entry at `./plugins/tuturuuu`.

After changing marketplace metadata or plugin files, restart Codex. Then open
the plugin directory, choose the `Tuturuuu Platform` marketplace, and install the
`Tuturuuu` plugin.
