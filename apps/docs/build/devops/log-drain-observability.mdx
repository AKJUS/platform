...
Queued deployment rows only advance while the blue/green deployment watcher is live. If the Projects tab shows a queued project and the watcher is missing, offline, or stale, the queue is only persisted state; restart or recreate the Docker web watcher so it can consume the queued record.
