...
- Compose files default `SRH_TOKEN` to `platform-local-redis-token` when
  `UPSTASH_REDIS_REST_TOKEN` is not exported. This keeps watcher and cron
  recovery commands from failing during Compose interpolation before the
  recovery service can start.
