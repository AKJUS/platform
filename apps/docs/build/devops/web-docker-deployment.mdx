...
4. Builds and starts the shared `markitdown`, `storage-unzip-proxy`, and
   `web-cron-runner` helper containers alongside the target color.
...
1. Reads the built-in `platform` project from the log-drain Postgres project registry when `PLATFORM_LOG_DRAIN_DATABASE_URL` is configured. The seeded branch is `production`, but operators can change it from Infrastructure → Monitoring → Projects.
2. If the selected project branch differs from the current checkout, fetches and checks out that branch only when the worktree is clean. Dirty worktrees are reported as blocked instead of being force-switched.
...
13. Runs `bun serve:web:docker:bg` automatically after a successful fast-forward
   pull.
14. If watcher runtime code such as `scripts/watch-blue-green-deploy.js`,
