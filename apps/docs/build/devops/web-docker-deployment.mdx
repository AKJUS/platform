...
```bash
bun serve:web:docker:bg -- --build-memory 6g --build-cpus 2 --build-max-parallelism 1
```

Current root-script defaults:

- `bun serve:web:docker` defaults to `--build-memory 6g --build-cpus 2 --build-max-parallelism 1`
- `bun serve:web:docker:bg` defaults to `--build-memory 6g --build-cpus 2 --build-max-parallelism 1`
...
- Docker web builds use `bun run build:web:docker`, which keeps the normal web
  build dependency graph but caps the web app's V8 heap at 4 GB instead of the
  local 8 GB build default.
...
- If BuildKit fails with `ResourceExhausted` or `cannot allocate memory`, lower
  `--build-memory` and `--build-max-parallelism` before retrying the same
  commit.
