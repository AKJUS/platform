...
Use `PRONUNCIATION_ASSESSOR_PRELOAD=true` when cold-start latency matters, or
leave it false so the first request lazily loads the selected model. On shared
hosts, keep `PRONUNCIATION_ASSESSOR_MAX_LOADED_MODELS=1` and tune
