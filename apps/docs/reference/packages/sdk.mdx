...
designated for the Tuturuuu CLI. The CLI stores that session in the OS app
config directory:
...
The CLI checks the npm registry at most once per day and writes a notice to
stderr when a newer `tuturuuu` version is available. This keeps `--json` output
machine-readable for agents. Use `--no-update-check` on a single command, or set
`TUTURUUU_DISABLE_UPDATE_CHECK=1`, to disable the check.

The v1 task surface covers workspaces, boards, lists, tasks, labels, projects,
relationships, moves, and bulk task updates. Add `--json` when another agent or
