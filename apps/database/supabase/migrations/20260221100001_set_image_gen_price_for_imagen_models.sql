--------------------------------------------------------------------------------
-- Store image_count in transactions so image_generation deductions show usage
-- (admin can display "N images" instead of "0 tokens").
--------------------------------------------------------------------------------
ALTER TABLE public.ai_credit_transactions
  ADD COLUMN IF NOT EXISTS image_count INTEGER;

-- Recreate deduct_ai_credits to write image_count into the ledger
...
BEGIN
  ...
  INSERT INTO public.ai_credit_transactions
    (ws_id, user_id, balance_id, execution_id, chat_message_id,
     ...
     input_tokens, output_tokens, reasoning_tokens, image_count, metadata)
  VALUES
    (CASE WHEN v_balance.ws_id IS NOT NULL THEN p_ws_id ELSE NULL END,
     ...
     p_input_tokens, p_output_tokens, p_reasoning_tokens, p_image_count, p_metadata);
...
--------------------------------------------------------------------------------
-- Expose image_count in admin transaction list (for image_generation rows)
--------------------------------------------------------------------------------
