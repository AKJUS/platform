-- Cap maxOutputTokens by remaining credits in check_ai_credit_allowance
-- Database layer: prevent over-generation while keeping deductions uncapped for observability
...
BEGIN
  ...
  -- Compute effective max output tokens (plan + model limits)
  ...
  -- *** NEW: Cap maxOutputTokens by affordable tokens from remaining credits ***
  -- Uses real gateway pricing from v_model (already fetched above)
  -- Formula: credits = (tokens * price_per_token / 0.0001) * markup
  --        → tokens = (remaining * 0.0001 / markup) / price_per_token
